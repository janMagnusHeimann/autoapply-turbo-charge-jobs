...
export class GoogleScholarService {
  ...
  static async scrapeScholarProfile(profileUrl: string): Promise<{
    ...
    try {
      ...
      if (!htmlContent) {
        console.log('🤖 All proxies failed, using backend to scrape your actual Google Scholar profile...');
        
        // Fallback to sample data when proxies fail
        console.log('📚 Using sample publication data as fallback...');
        return this.generateSamplePublications(authorId);
      }
